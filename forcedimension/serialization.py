...
class BaseAngles(pydantic.BaseModel):
    x: float = 0.0
    y: float = 0.0
    z: float = 0.0
...
class HapticDeviceSpecs(pydantic.BaseModel):
    ...
    @pydantic.validator('serial_number')
    @classmethod
...
class HapticDeviceConfig(pydantic.BaseModel):
    class Config:
        json_encoders = {
            containers.Vector3: lambda v: list(v)
        }

        json_decoders = {
            containers.Vector3: lambda v: containers.Vector3(v)
        }
    ...
    class RegulatorConfig(pydantic.BaseModel):
        ...
        @pydantic.validator('max_motor_ratio')
        @classmethod
    ...
    @pydantic.validator('standard_gravity')
    @classmethod
